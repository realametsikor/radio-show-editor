...
export default function FileUpload({ onUploadComplete }: FileUploadProps) {
  ...
  const [useCustomMusic, setUseCustomMusic] = useState(false); // Decoupled from mood
  ...
  const handleUpload = async () => {
    ...
    if (useCustomMusic && customMusicFile) {
  ...
  const isMissingCustomFiles = 
    ...
    (useCustomMusic && !customMusicFile);
        ...
        <div className="glass-card rounded-2xl p-5 border border-gray-800 bg-[#13131A]">
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="story">Story / Narration (Calm & Immersive)</option>
            </select>
          ...
          {/* Independent custom music toggle — works with ANY vibe, including Story */}
          <label className="mt-3 flex items-center gap-2 text-xs text-gray-400 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={useCustomMusic}
              onChange={(e) => setUseCustomMusic(e.target.checked)}
              className="h-3.5 w-3.5 rounded border-gray-600 bg-gray-900 text-indigo-500 focus:ring-indigo-500"
            />
            Use my own uploaded music instead of the AI-picked track
          </label>
        </div>
