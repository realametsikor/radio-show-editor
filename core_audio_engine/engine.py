...
def run_pipeline(
    ...
    *,
    ...
    from core_audio_engine.mixer    import mix_with_ducking, mix_narration, add_natural_pauses

    # Story/narration mode uses a calm, steady bed instead of the
    # two-host podcast "cinematic" pacing (no injected breaks/swells).
    narration_mode = mood == "story"
    ...
    try:
        ...
        # Narration mode gets shorter bumpers — long dead air feels off on a story
        intro_pad = AudioSegment.silent(duration=3000 if narration_mode else 6000)
        outro_pad = AudioSegment.silent(duration=5000 if narration_mode else 10000)
